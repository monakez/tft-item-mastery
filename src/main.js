...
async function init() {
  ...
  const COMBINED_ITEMS = items.filter(item => item.kind===3).filter(item=>!item.recipe.some(i=>emblemIdSet.has(i)));

  const COMBINED_EMBLEM_ITEMS = items.filter(item => item.kind===3).filter(item=>item.recipe.some(i=>emblemIdSet.has(i)));
  ...
  const allItems=new ItemSet([...BASE_ITEMS,...EMBLEM_ITEMS],[...COMBINED_EMBLEM_ITEMS,...COMBINED_ITEMS])
