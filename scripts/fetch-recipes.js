...
async function fetchCompositeRecipes() {
  ...
  for (const [baseName] of Object.entries(BASE_ITEM_NAME_TO_ID)) {
    ...
    try {
      ...
      $('th:contains("Upgrades")').closest('table').find('.centered-grid-icon').each((i, el) => {
        const components = [];
        let compositeName = '';
        
        $(el).find('.inline-image').each((j, compEl) => {
...
async function parseCompositeItem(itemName) {
  ...
  try {
    ...
    $('div[style*="display:inline-block; box-shadow"]').nextAll('div').each((i, div) => {
